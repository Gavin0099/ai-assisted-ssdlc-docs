...
3. **比較維度 (Comparison Dimensions)**:
   - **Target Metadata Diff**: 比對 `repo`、`commit`、`manifest_digest`、`corpus_digest` 等出處元數據。
   - **Task Status Diff**: 依據 NIST SSDF 任務定義與兩邊 findings，客觀標示各任務的狀態為：
     - `ADDED`: 僅存在於 Target。
     - `REMOVED`: 僅存在於 Baseline。
     - `MODIFIED`: 兩邊皆存在，但其 `coverage_verdict`、`company_source_ref`、`company_statement`、`basis` 或 `cannot_claim` 有所異動。
     - `UNCHANGED`: 兩邊皆存在且內容完全一致。
   - **Task Ordering**: 輸出結果一律先依 NIST SSDF 任務 ID 字典序排序。
